...
class Parser_Run1(Parser):
    ...
    def parse_consent_line_to_dictionary(self,line):
        ...
        time_stamp_event = tokens[0]
        time_stamp = time_stamp_event[:12]
        event = tokens[0].rsplit(self.separator2)[1]
#         event = self.extract_event(line,self.separator1, self.separator2)        
        ...
        if(event=="CONSENT"):
            ...
            try:
                ...
                tcount = results[1] + 1 
                tuple_line["gender"] = re.split(self.separator2,tokens[tcount])[1]
                tcount += 1
